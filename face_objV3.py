    ...
    def regFace(self, params, frame): #register colors on this face (9 boxes - thus 9 smaller cells) 
        for r in range(3):
            for c in range(3):
                ...
                #two algorithhms to find the color of a cell
        ...
        self.centerColor=self.cells[1][1].returnColor() #Identify the center cell color on this face
...
#Monitor our scanning progress on a different frame
def draw_scanned_cube(cube_scanned, the_cube, id):
    ...
    for i , face in enumerate(the_cube[0:6]):#['U','R','F','D','L','B']
        #print(i)
        thiscolor=None
        for r in range(3):
...
#generate a cube seq for the solver
...
colorToSeq={'b':'U','r':'R', 'w':'F', 'g':'D',  'o':'L', 'y':'B'} #default - update after we registered the cube
